...
def hamlet() -> Dict:
    ...
    return dict(
        ...
        year=1996,
        tmdb_id=142,
        original_title="Omlet",
        release_date=database.datetime.date(1996, 6, 6),
...
def dreams() -> Dict:
    ...
    return dict(
        title="Akira Kurosawa's Dreams", director="Kurosawa", minutes=119, year=1972
    )
...
def loaded_database(hamlet, solaris, dreams, revanche):
    ...
    database.add_movie_tag_link(
        "blue", database.MovieKeyTypedDict(title="Hamlet", year=1996)
    )
    database.add_tag("yellow")
    database.add_movie_tag_link(
        "yellow", database.MovieKeyTypedDict(title="Revanche", year=2008)
    )
    database.add_tag("green")
    database.add_movie_tag_link(
        "green", database.MovieKeyTypedDict(title="Revanche", year=2008)
    )
    database.add_movie_tag_link(
        "green", database.MovieKeyTypedDict(title="Solaris", year=1972)
    )
...
def test_init_database_access_with_new_database(connection):
    ...
    with database._session_scope() as session:
        today = (
            session.query(database.MoviesMetaData.value)
...
def test_add_movie_with_empty_title_string_raises_value_error(
    connection, session, monkeypatch
):
    expected = "Year string expected but got empty string."
...
class TestFindMovie:
    ...
    def test_search_movie_year(self):
        test_year = 1996
        movies = database.find_movies(
            database.FindMovieTypedDict(year=[str(test_year)])
        )
...
class TestReplaceMovie:
    ...
    NEW_MOVIE: database.MovieTypedDict = dict(
        title="New Movie", year="2042", director="Starchild"
    )
    ...
    def test_replace_all_fields(self):
        ...
        new_movie = database.MovieTypedDict(
            title="Omlet", director="Starchild", minutes=42, year=1942, notes="fortytwo"
        )
    ...
    @staticmethod
    def context_wrapper(
        old_movie: database.MovieKeyTypedDict, new_movie: database.MovieTypedDict
    ) -> Callable:
...
def test_delete_movie_by_title_and_length(loaded_database):
    database.del_movie(
        database.FindMovieTypedDict(title="Solaris", minutes=["", "169"])
    )
    movies = database.find_movies(dict(title="Solaris"))
    assert movies == []
...
def test_movie_tags(loaded_database, revanche):
    title_year = database.MovieKeyTypedDict(
        title=revanche["title"], year=revanche["year"]
    )
...
class TestTagOperations:
    ...
    def test_add_links_to_movies(self, session):
        ...
        movies = session.query(database.Movie.title).filter(
            database.Movie.tags.any(tag=test_tag)
        )
    ...
    def test_edit_tag(self, session):
        ...
        old_tag_id, tag = (
            session.query(database.Tag.id, database.Tag.tag)
    ...
    def test_edit_tag_raises_exception_database_search_found_nothing(
    ...
    def test_edit_tag_logs_database_search_found_nothing(self, session, monkeypatch):
        ...
        assert calls == [
            (
                "An incomplete database session has been rolled back because of exception:\n"
                "NoResultFound"
            ),
    ...
    def test_edit_movies_tag(self, session):
        ...
        movies = database.find_movies(
            database.FindMovieTypedDict(
                title=title_year["title"], year=[str(title_year["year"])]
            )
        )
...
def test_logging_called(monkeypatch):
    ...
    with pytest.raises(DatabaseTestException):
        with database._session_scope() as session:
            session.incorrect_function()
    assert log.info_calls[0] == (
        "An incomplete database session has been rolled back "
