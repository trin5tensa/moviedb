...
class TestMainWindow:
    ...
    def test_main_window_initialization(self, monkeypatch):
        ...
        monkeypatch.setattr(mainwindow.handlers, 'EscapeKeyDict', mock_escape_key_dict := MagicMock())
        ...
        with main_window(monkeypatch) as cut:
            ...
            with check:
                cut.parent.bind_all.assert_has_calls([call('<Escape>', mock_escape_key_dict().escape()),
    ...
    def test_place_menubar(self, monkeypatch):
        ...
        with main_window(monkeypatch) as cut:
            ...
            with check:
                cut.parent.protocol.assert_called_once_with('WM_DELETE_WINDOW', cut.tk_shutdown)
            with check:
                cut.parent.createcommand.assert_has_calls(
                    [call('tk::mac::Quit', cut.tk_shutdown),
                     call('tk::mac::ShowPreferences', mainwindow.handlers.settings_dialog)])
            with check:
            ...
            with check:
                cut.moviedb_menu.assert_has_calls([
                    call.add_command(label='About ' + TEST_TITLE + '…', command=mainwindow.handlers.about_dialog),
                    call.add_separator(),
                    call.add_command(label='Settings for Moviedb…', command=mainwindow.handlers.settings_dialog),
                    call.add_separator(),
            ...
            with check:
                cut.edit_menu.assert_has_calls([
                    call.add_command(label='Cut', command=cut.cut_command, accelerator='Command+X'),
                    call.add_command(label='Copy', command=cut.copy_command, accelerator='Command+C'),
                    call.add_command(label='Paste', command=cut.paste_command, accelerator='Command+V'),
            ...
            with check:
                cut.movie_menu.assert_has_calls([
                    call.add_command(label='Add Movie…', command=mainwindow.handlers.add_movie),
                    call.add_command(label='Edit Movie…', command=mainwindow.handlers.edit_movie),
                    call.add_command(label='View Movie…', command=mainwindow.handlers.edit_movie),
                    call.add_command(label='Delete Movie…', command=mainwindow.handlers.edit_movie),
                    call.add_separator(),
                    call.add_command(label='Add Tag…', command=mainwindow.handlers.add_tag),
                    call.add_command(label='Edit Tag…', command=mainwindow.handlers.edit_tag),
                    call.add_command(label='Delete Tag…', command=mainwindow.handlers.edit_tag),
                    ])
