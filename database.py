...
def connect_to_database(filename: str = database_fn):
    ...
    with _session_scope() as session:
        ...
        # Code for a new database
            ...
            session.add_all(
                [
                    MoviesMetaData(name="version", value="1"),
...
def find_movies(criteria: FindMovieTypedDict) -> List[MovieUpdateDef]:
    ...
    movies = [
        MovieUpdateDef(
            ...
            year=movie.year,
            notes=movie.notes,
            tags=[tag.tag for tag in movie.tags],
        )
        for movie in query
    ]
    ...
    try:
        with _session_scope() as session:
            ...
            # The old movie has gone, so it can't be modified.
                ...
                msg = (
                    f"The movie was not found in the database. It may have been deleted by an "
                    f"external process."
                )
                logging.error(msg)
...
def movie_tags(title_year: MovieKeyTypedDict) -> List[str]:
    ...
    with _session_scope() as session:
        tag_names = (
            ...
            .join(Tag.movies)
            .filter(Movie.title == title_year["title"])
        )
...
def add_movie_tag_link(tag: str, movie: MovieTD):
...
def edit_movie_tag_links(
    movie: MovieKeyTypedDict, old_tags: Iterable[str], new_tags: Iterable[str]
):
...
class MoviesMetaData(Base):
    ...
    def __repr__(self):  # pragma: no cover
        return (
            self.__class__.__qualname__ + f"(name={self.name!r}, value={self.value!r}"
        )
...
class Movie(Base):
    ...
    def __repr__(self):  # pragma: no cover
        return (
            self.__class__.__qualname__ + f"(title={self.title!r}, year={self.year!r}, "
...
class Tag(Base):
    ...
    id = Column(
        sqlalchemy.Integer, sqlalchemy.Sequence("tag_id_sequence"), primary_key=True
    )
    ...
    def __repr__(self):  # pragma: no cover
        return self.__class__.__qualname__ + f"(tag={self.tag!r})"
...
class Review(Base):
    ...
    id = Column(
        sqlalchemy.Integer, sqlalchemy.Sequence("review_id_sequence"), primary_key=True
    )
    ...
    movies = relationship(
        "Movie", secondary="movie_review", back_populates="reviews", cascade="all"
    )
...
def _session_scope() -> Generator[Session, None, None]:
    ...
    try:
        ...
        session.rollback()
        msg = (
            f"An incomplete database session has been rolled back because of exception:\n"
...
def _build_movie_query(
    session: Session, criteria: MovieSearch
